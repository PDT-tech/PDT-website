...
016 | DONE   | Music Library: 403 error observed in production on 2026-04-17 when downloading from members/music.html. Root cause: dlUrl() built direct drive.google.com URLs, bypassing the proxy. Fix: added type=download endpoint to drive-music.js that proxies file content server-side via service account token. Added validateSession() to all three endpoints — unauthenticated requests now get 401. Updated music.html to route all downloads through proxy with Bearer token auth. Fixed 2026-04-18. Files: netlify/functions/drive-music.js, members/music.html.
