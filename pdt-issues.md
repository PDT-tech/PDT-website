...
016 | DONE   | Music Library: 403 error observed in production on 2026-04-17 when downloading from members/music.html. Root cause: download links were direct drive.google.com URLs — browser hit Google unauthenticated. Fix: added type=download endpoint to drive-music Netlify function (server-side fetch with service account token, base64 response); all three endpoints now require Supabase session validation; client uses fetch+Blob approach so token travels in Authorization header only. Fixed 2026-04-18. Files: netlify/functions/drive-music.js, members/music.html.
